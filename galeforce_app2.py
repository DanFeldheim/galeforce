...
class Flow_Control():
    ...
    def all_calls(self):
        ...
        try:
            ...
            splitter = data.split(other)
            
        except:
            st.error('Please make sure your spreadsheet has the correct format and column names.')
...
class Upload():
    ...
    def move_other(self, data_df):
        ...
        for index, row in data_df.iterrows():
            ...
            if pd.isnull(row[column_to_check]):  
                data_df.loc[index, column_to_check] = row[column_to_move_from]
    ...
    def split(self, data_df):
        ...
        self.registrant_df = data_df.iloc[:, [1,2] + list(range(6, data_df.shape[1]))]
        ...
        # Create a dict to hold the individual registrant dataframes
        ...
        for number in range(1, max_registrant_number + 1):
            ...
            subset_df = self.registrant_df[['Church'] + 
        ...
        return admin_info, subset_df_dict
    ...
    def agg_dataframe(self, admin_df, data_df):
        ...
        self.final_df = data_df.sort_values(by = 'Church', ascending=True)
        # Drop rows where all columns except the first one are None
        self.final_df = self.final_df.dropna(subset = self.final_df.columns[1:], axis = 0, thresh = 6)
        # Merge the admins and registrants
        self.final_df = pd.merge(self.final_df, admin_df, on='Church', how='left')

        # Reorder the columns
        self.final_df = self.final_df[['Church', 
                                      'Admin Name', 
                                      'Admin Email',
                                      'Admin Phone',
                                      'Registrants',
